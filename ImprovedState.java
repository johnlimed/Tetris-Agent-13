...
public class ImprovedState {
	...
	// was placed
	private int[][] curField = new int[ROWS][COLS]; // the current state of the
													// grid
	...
	// initialize legalMoves array for each piece
	{
		...
		for (int i = 0; i < N_PIECES; i++) {
			...
			// allocate space
			legalMoves[i] = new int[n][2]; // n is number of legalMoves for this
											// piece, second[] has 2 values
											// containing: ORIENT/SLOT
			// for each orientation
			n = 0;
			for (int j = 0; j < pOrients[i]; j++) {
				// for each slot
				for (int k = 0; k < COLS + 1 - pWidth[i][j]; k++) {
					legalMoves[i][n][ORIENT] = j; // i is the piece type, j =
													// number of possible
													// orients
	...
	// the deepCopy parameter controls whether a deep copy of the objects are
	// done, which is required for modification not to affect other objects
	// Being able to control whether copies are deep or not is primarily a
	...
	// returns a new object representing the state of the game if a particular
	// move was made
		...
		for (int r = height + pHeight[nextPiece][orient] - 1; r >= height; r--) {
			...
			if (full) {
				...
				for (int c = 0; c < COLS; c++) {
					...
					// lower the top
					newTop[c]--;
					while (newTop[c] >= 1 && newField[newTop[c] - 1][c] == 0)
						newTop[c]--;
				}
			}
		}
	...
	// these functions execute the move and modifies the board; they're
	// essentially copied directly from State
		...
		for (int r = height + pHeight[nextPiece][orient] - 1; r >= height; r--) {
			...
			if (full) {
				...
				for (int c = 0; c < COLS; c++) {
					...
					// lower the top
					top[c]--;
					while (top[c] >= 1 && curField[top[c] - 1][c] == 0)
						top[c]--;
				}
			}
		}
	...
	// tests if the current state information in 2 ImprovedState objects are
	// equal
